 ...
 *
 *      history: 13 nov 2019 fixed mem leak in table_export (10q phil love)
 *      history: 13 nov 2019 added 'unexport' to free new environ
 ...
 * hist: 2019-11-13 if tp is NULL then free current environ and restore orig
 */
...
table_export(symtab_t *tp)
{
	...
	if ( env_orig == NULL ){
		...
		free(environ);		/* old memory leak: 10q p.love	*/
	}

	/* if tp == NULL and env_orig != NULL then restore orig		*/
